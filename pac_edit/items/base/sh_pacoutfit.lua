...
function ITEM:OnLoadout()
	...
	if IsValid(owner) and owner:IsPlayer() and owner:Alive() and self:GetData("equip", false) then
		self:Equip(owner)
	end
end
...
function ITEM:Equip(client)
	...
	for _, v in pairs(items) do
		if (v.id != self.id) then
			...
			if (itemTable.pacData and v.outfitCategory == self.outfitCategory and itemTable:GetData("equip")) then
				client:NotifyLocalized(self.equippedNotify or "outfitAlreadyEquipped")
	...
	if self.pacData or self.pacDataFemale or self.pacDataMale then
		if client:IsFemale() and self.pacDataFemale then
			client:AddPart(self.uniqueID .. "female", self)
		elseif !client:IsFemale() and self.pacDataMale then
			client:AddPart(self.uniqueID .. "male", self)
		else
